...
export interface ReturnPolicyFormDrawerProps {
  isOpen: boolean
  onClose: () => void
...
// Standard condition tags for inspection/acceptance
const AVAILABLE_CONDITIONS = [
  ...
  { code: 'unwashed', labelKm: 'មិនទាន់បោកគក់ / ប្រើប្រាស់', labelEn: 'Unwashed / Unworn' },
  { code: 'no_scratches', labelKm: 'គ្មានស្នាមឆ្កូតលើអេក្រង់/តួ', labelEn: 'No Scratches' },
  { code: 'all_in_box_accessories', labelKm: 'គ្រឿងបន្លាស់គ្រប់ក្នុងប្រអប់', labelEn: 'All Accessories Present' },
]

export const ReturnPolicyFormDrawer: React.FC<ReturnPolicyFormDrawerProps> = ({
  isOpen,
  onClose,
  editingPolicy,
  categories,
  ...
  // Sync state on open / editingPolicy change
  useEffect(() => {
    if (isOpen) {
      setNameError(null)
      if (editingPolicy) {
    ...
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
          ...
          >
            {/* Top Gradient Accent */}
            <div className="h-1 bg-gradient-to-r from-primary via-blue-500 to-indigo-500 w-full shrink-0" />

            {/* ── 1. GLOBAL DRAWER HEADER ────────────────────────────────────────── */}
            <div className="flex items-center justify-between px-5 sm:px-6 py-4 border-b border-border/80 bg-muted/20 dark:bg-slate-900/50 backdrop-blur-md shrink-0">
              <div className="flex items-center gap-3 min-w-0 flex-1 pr-3">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border shadow-2xs bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20">
                  <ShieldCheck size={20} />
                </div>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-base sm:text-lg font-bold text-foreground dark:text-slate-100 leading-snug">
                      {isEdit
                  ...
                  <p className="text-xs text-muted-foreground dark:text-slate-400 mt-0.5 truncate max-w-xl">
                    {t(
                      'policies.subtitle',
              ...
              <CloseButton onClose={onClose} size="md" variant="default" className="shrink-0" />
            </div>

            {/* ── 2. SCROLLABLE FORM BODY ────────────────────────────────────────── */}
            <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-5">
              <form id="return-policy-drawer-form" onSubmit={handleSubmit} className="space-y-5">
                {/* ── SECTION 1: ព័ត៌មានទូទៅនៃគោលការណ៍ ── */}
                <div className="rounded-2xl border border-border/80 bg-muted/20 dark:bg-slate-900/40 p-4 sm:p-5 space-y-4 shadow-2xs">
                  ...
                  <div className="space-y-3.5">
                    ...
                    {/* Store-Wide Default Toggle Banner */}
                    <div
                ...
                {/* ── SECTION 2: កាលកំណត់ និងកម្រៃសេវា ── */}
                <div className="rounded-2xl border border-border/80 bg-muted/20 dark:bg-slate-900/40 p-4 sm:p-5 space-y-4 shadow-2xs">
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground/90">
                ...
                {/* ── SECTION 3: លក្ខខណ្ឌ និងការអនុញ្ញាត ── */}
                <div className="rounded-2xl border border-border/80 bg-muted/20 dark:bg-slate-900/40 p-4 sm:p-5 space-y-4 shadow-2xs">
                  <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-muted-foreground/90">
            ...
            {/* ── 3. GLOBAL DRAWER FOOTER ────────────────────────────────────────── */}
            <div className="border-t border-border/80 bg-muted/20 dark:bg-slate-900/50 backdrop-blur-md shrink-0">
              <ModalFooter
                onCancel={onClose}
...
export default ReturnPolicyFormDrawer
